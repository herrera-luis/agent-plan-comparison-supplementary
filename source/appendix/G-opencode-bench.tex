...
auth, model-string verification, and the drop / redact / guard recipe),
the per-benchmark MCP picker configurations (simple vs complex tool
surface), and the headless wrapper are retained in the study's private
operational record.
...
OpenCode cells. Across all three benchmarks OpenCode's cross-model
average ($3.701$) sits within the ${\sim}0.1$-point cluster with
OpenSpec ($3.798$) and Ralph ($3.728$) (Table~\ref{tab:aggregate}): the
blog bench has no Jira ticket and no rich external MCP for OpenCode to
use (Param~2 and Param~6 sit at the no-external-source baseline), so its
blog cell is lowest of the three there under both models ($3.540$
cross-model). OpenCode wins the video-support bench outright ($3.955$
cross-model vs Ralph's $3.855$ and OpenSpec's $3.830$).}
...
\texttt{openspec} ($3.765$); on the blog bench OpenCode is
...
three-benchmark average therefore places OpenSpec ($3.798$), Ralph
($3.728$), and OpenCode ($3.701$) within a single ${\sim}0.1$-point
cluster (Section~\ref{sec:results}) rather than in a definitive order,
and what little separation there is reflects benchmark composition: on
the one bench where integration demand makes the canonical vs Cline
pairing directly comparable, OpenCode's native pairing is on top, so the
Cline-pairing confound is not \emph{inflating} OpenSpec's or Ralph's
scores past OpenCode's. The
...
counts) remain literally true, but the small cross-tool differences in
the three-benchmark average are determined by the benchmark composition
(in particular the breadth-oriented blog bench), not by the driver.
...
OpenCode is thus the most token-intensive tool under Opus~4.8 across
these benches, though the extreme blog-bench figure (\$49.57) rests on
a single unrepeated run and should be treated as a candidate anomaly
pending replication, and the cheapest tool on the video-support bench
under GPT-5.5; the cost ordering across tools cannot be read without
...
(\texttt{OPENCODE\_PLAN.md}~Section~2.3) and Task~2 broadens the character
